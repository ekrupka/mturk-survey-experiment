...
!(function($) {
	function Tokens( elt ) {
		...
		this.$tokens = $( elt ); //
		this.$tokenSliderWrapper = $( '.token-slider-wrapper' );	
		this.$tokenHeader = $( '.token-header' ); // token header
		...
		// compile teampltes
		this.tokenTemplate = Handlebars.compile( this.$tokenTemplate );
		this.tokenTemplateTen = Handlebars.compile( this.$tokenTemplateTen );
		this.tokenTemplateZero = Handlebars.compile( this.$tokenTemplateZero );
		...
		this.givingText = 'You are currently giving ' + this.tokenSpanGiving;
		this.takingText = 'You are currently taking ' + this.tokenSpanTaking;
		this.tokenHelpEndText = '</span> tokens';
	...
	Tokens.prototype = {
		...
		// consider moving this logic to next question - how to not make repetitive
		initTokenHeader: function() {
			var template = null;

			if ( this.youOwnTokens === 10 ) {
				template = this.tokenTemplateTen( contextTokens );
			} else if ( this.youOwnTokens === 0 ) {
				template = this.tokenTemplateZero( contextTokens );
			} else {
				template = this.tokenTemplate( contextTokens );
			}

			this.$tokenHeader.after( template );
		},
